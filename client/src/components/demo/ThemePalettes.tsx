...
import { ColorPreview, PresetColors, useColorState } from '@/lib/color-picker';
...
export function ThemePalettes() {
  const [selectedTheme, setSelectedTheme] =
    useState<keyof typeof THEME_PRESETS>('tailwind');
  const [selectedColor, setSelectedColor] = useState<string>(
    THEME_PRESETS.tailwind[0]
  );
  const { colorValue, setFromString } = useColorState(selectedColor);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                oklch({(colorValue.oklch.L * 100).toFixed(0)}%{' '}
                {colorValue.oklch.C.toFixed(2)} {colorValue.oklch.h.toFixed(0)})
              </p>
            </div>
          </div>
